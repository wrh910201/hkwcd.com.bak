...
class SystemAction extends CommonAction {
	...
	public function site() {
		...
		$this->styleDirList = getFileFolderList(APP_PATH . C('APP_GROUP_PATH') . '/Index/Tpl/' , 1);
	...
	public function qqset() {
		if (IS_POST) { 
			if(F('config.qqset',$_POST,CONF_PATH)) {
				$this->success('修改成功',U(GROUP_NAME. '/System/qqset')); 
			}else {
	...
	public function adset() {
		if (IS_POST) { 
			...
			$cfg_ad_num = strtoupper($_POST['cfg_ad_num']);
			if (empty($cfg_ad_num)) {
				$this->error('条数不能为空');
			}
			
			$cfg_ad_width = strtoupper($_POST['cfg_ad_width']);
			if (empty($cfg_ad_width)) {
				$this->error('宽度不能为空');
			}
			
			$cfg_ad_height = strtoupper($_POST['cfg_ad_height']);
			if (empty($cfg_ad_height)) {
				$this->error('高度不能为空');
			}
			 
			if(F('config.adset',$_POST,CONF_PATH)) {
				$this->success('修改成功',U(GROUP_NAME. '/System/adset')); 
			}else {
