...
var config = {
    ...
    right: -232,
    api: {
        friend: '/operation/admin/getOperators', //好友列表接口
        group: '/static/static/layer/layim/data/group.json', //群组列表接口
        chatlog: '/static/static/layer/layim/data/chatlog.json', //聊天记录接口
        groups: '/static/static/layer/layim/data/groups.json', //群组成员接口
        sendurl: '' //发送消息接口
    },
    user: { //当前用户信息
        name: operator,
...
xxim.popchat = function(param){
    ...
    log.success = function(layero, index){
        // layer.setMove();
        console.log("popchat");
        ...
        $(".layui-layer-content").attr("style", 'overflow:visible');
    ...
    log.html = '<div class="layim_chatbox" id="layim_chatbox">'
            ...
            +'    <a href="'+ param.url +'" class="layim_face" target="_blank"><img style="height: 60px; width: 60px" src="'+ param.face +'" ></a>'
    ...
    if(config.chatings < 1){
        ...
        layer.open({
            ...
            success: function(layero, index){
                ...
                log.success(layero, index);
                config.box = layero;
                config.index = index;
            }
        })
    } else {
        // console.log("chatings egt 1")
...
xxim.popchatbox = function(othis){
    ...
    if(config.index === -1){
        ...
        // node.layimMin.hide();
        // if( is_restore || ) {
        //     layer.restore(config.index);
        // }
        console.log(node.layimMin);
        console.log($(node.layimMin));
        console.log(node.layimMin.is(":visible"))
        console.log($(node.layimMin).is(":visible"));
        if( $(node.layimMin).is(":visible") ) {
            node.layimMin.hide();
            layer.restore(config.index);
        }
        config.box.find('.layui-layer-max').hide();
        config.box.find('.layui-layer-min').hide();
    }
};
...
//接收消息
xxim.receive = function(message) {
    //弹出聊天窗
    var node = xxim.node;
    var log = {};
    var othis = $(".xxim_childnode[data-id="+message.from+"]");
...
//        alert("收到服务端的消息：" + e.data);
    ...
    ws.onclose = function (e) {
        window.clearInterval(config.timmer);
    };
};

//渲染在线成员
xxim.renderOnline = function(datas) {
    var node = xxim.node;
    var myf = node.list.eq(0);
    // console.log("renderOnline");
    // console.log(datas);
    if( datas ) {
        if (datas.status === 1) {
            var i = 0, myflen = datas.data.length, str = '', item, index = 0;
            if (myflen >= 1) {
                if (index !== 2) {
                    for (; i < myflen; i++) {
                        str += '<li data-id="' + datas.data[i].id + '" class="xxim_parentnode">'
...
xxim.registerTimer = function() {
    var timmer = window.setInterval(this.ping, 8000);
    config.timmer = timmer;
}
...
//有用户上线
xxim.login = function(datas) {
    var data = datas.data;
    var parent_id = data.id;
    var object = $(".xxim_parentnode[data-id="+parent_id+"]");
    if( object ) {
        var item = data.item;
        if( !$(".xxim_childnode[data-id="+item.id+"]") ) {
            var str = '<li data-id="' + item.id + '" class="xxim_childnode" type="' + (index === 0 ? 'one' : 'group') + '"><img src="' + item.face + '" class="xxim_oneface"><span class="xxim_onename">' + item.name + '</span></li>';
            $(".xxim_parentnode[data-id="+parent_id+"] .xxim_chatlist").append(str);
        }
    } else {
        var str = '<li data-id="' + parent_id + '" class="xxim_parentnode">'
            + '<h5><i class="fa fa-caret-right"></i><span class="xxim_parentname">' + data.name + '</span><em class="xxim_nums">（1）</em></h5>'
            + '<ul class="xxim_chatlist">';
            + '</ul>';
        $(".xxim_list").append(str);
        var item = data.item;
        if( !$(".xxim_childnode[data-id="+item.id+"]") ) {
            var str = '<li data-id="' + item.id + '" class="xxim_childnode" type="' + (index === 0 ? 'one' : 'group') + '"><img src="' + item.face + '" class="xxim_oneface"><span class="xxim_onename">' + item.name + '</span></li>';
            $(".xxim_parentnode[data-id="+parent_id+"] .xxim_chatlist").append(str);
...
//有用户下线
xxim.logout = function(datas) {

}


xxim.getNowFormatDate = function() {
    var date = new Date();
    var seperator1 = "-";
    var seperator2 = ":";
